...
- [ ] **Phase 4** — Real payment (Razorpay). **Blocked on the user creating a Razorpay account** (needs PAN, bank account for payouts, business category — KYC can take hours to days, but test-mode API keys are available immediately after signup, before KYC finishes). Not started; picking this back up just needs the user to hand over test-mode Key ID + Key Secret.
...
Phase 2 (availability) and Phase 3's core booking mechanics are both done. Both Phase 4 (Razorpay) and the Azure deploy validation are now blocked on the user creating accounts (see above) — **user chose to pursue the Azure deploy validation next**. Waiting on:
...
3. Once both exist: push this local repo to the new GitHub repo (needs the user's go-ahead first), then walk through the Azure Portal "Create Static Web App" wizard together (Free plan, GitHub source, **Next.js** build preset, app location `/`, api/output locations left blank per Microsoft's own guide) — then add `DATABASE_URL` and `ADMIN_API_KEY` as environment variables on the deployed resource (Configuration blade) since they're gitignored and won't come along with the push.

Worth raising with the user when this resumes: Phase 3 left a known gap (no real-time "new request" notification) and no cancellation endpoints yet — not blockers, just not done.
